...
class WishRepositoryImpl(
    ...
    // wish status
    override suspend fun setWishStatus(wishId: Long, status: WishStatusType): WishDomainModel? {
        return wishNetworkDataSource.setWishStatus(wishId = wishId, status = status)
    }
